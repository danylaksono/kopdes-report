...
- **_"dibangun di tanah kuburan"_** — built on burial ground. ESA WorldCover has
  no cemetery class at all, so [04](../04-siting-screen/) is structurally blind
  to it. OSM has the polygons.
- **_"dibangun di tengah sawah"_** — built in the middle of a paddy field. 04
  found **exactly one** cropland case in 2,500 candidates, and said so as a
  known limit: it ranks by _isolation_, and a shop in a paddy field sits right
...
- **16 of the 22 are in a _kota_, not a _kabupaten_.** These are large urban TPU
...
**The honest verdict: _"dibangun di tanah kuburan"_ is not supported as a
...
Distance to the nearest _pasar_ (`amenity=marketplace`), now measured exactly
...
ambiguity in a new place, and the evidence here leans _against_ the reassuring
...
in paddy fields."** The supportable sentence is _"N cooperatives are **recorded
at** a location inside a paddy field"_ — and if the ministry answers that the
...
- **The asymmetry rule, harder here than anywhere else.** OSM maps ~9,100 burial
  ...
  a miss is _no_ evidence. Every "distance to nearest X" column is an **upper

...
| ---------------------- | ----------------------- | --------- |
| NPWP (tax id)          | 80,977                  | **97.1%** |
| NIB (business license) | 60,813                  | **72.9%** |

Compliance is identical at every admin level (the sums are the same numbers
rolled up). Province spread of NIB: **99.6%** (DKI Jakarta) down to **5.6%**
(Papua Pegunungan); Papua Selatan has only **40.8%** NPWP. The registration
...
| ---------- | --------------- | ------------- |
| **NIB**    | 58,027 (69.85%) | 2,703 (3.25%) |
| **no NIB** | 22,316 (26.86%) | 23 (0.03%)    |

- **69.9% of villages hold the NIB and report no transaction.** The state has
...
- Village-level aggregation: a village counts as "NIB" if _any_ cooperative in
  it holds one. The 69.9% figure is therefore a lower bound on licensed-but-
